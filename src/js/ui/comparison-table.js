...
function createComparisonCardsElement(scenarios, options = {}) {
  ...
  scenarios.forEach((scenario) => {
    ...
    // Scenarios generated by generator.js always include totalWeeklySubsidy/totalWeeklyOutOfPocket/totalWeeklyCost.
    // The annualXxx / 52 fallbacks exist for backwards-compatible legacy scenario objects that may
    // only carry annual totals.
    const periodSubsidy = convertToPeriod(scenario.totalWeeklySubsidy || (scenario.annualSubsidy ?? 0) / 52, period);
    const periodOutOfPocket = convertToPeriod(scenario.totalWeeklyOutOfPocket || (scenario.annualOutOfPocket ?? 0) / 52, period);
    const periodFullCost = convertToPeriod(scenario.totalWeeklyCost || (scenario.annualCost ?? 0) / 52, period);
